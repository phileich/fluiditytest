...
public abstract class TOMSender implements ReplyReceiver, Closeable, AutoCloseable {
	...
	private int unorderedMessageSequence = 0; // sequence number for readonly
												// messages
	private CommunicationSystemClientSide cs; // Client side comunication system
	private Lock lock = new ReentrantLock(); // lock to manage concurrent access
												// to this object by other
												// threads
	...
	private void startsCS(int clientId) {
		...
		this.cs.setReplyReceiver(this); // This object itself shall be a reply
										// receiver
	 ...
	 *            Data to be multicast
	 * @param reqId
	 *            unique integer that identifies this request
	 * @param reqType
	 *            TOM_NORMAL, TOM_READONLY or TOM_RECONFIGURATION
	 */
	public void TOMulticast(byte[] m, int reqId, TOMMessageType reqType) {
		cs.send(useSignatures, viewController.getCurrentViewProcesses(),
				new TOMMessage(me, session, reqId, m, viewController.getCurrentViewId(), reqType));
	}
	...
	public void sendMessageToTargets(byte[] m, int reqId, int[] targets, TOMMessageType type) {
		...
		cs.send(useSignatures, targets, new TOMMessage(me, session, reqId, m, viewController.getCurrentViewId(), type));
	}
	 ...
	 *
	 * @param m
	 *            Data to be included in TOMMessage
	 *
