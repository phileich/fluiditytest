...
public class ServiceProxy extends TOMSender {
	 ...
	 *            used for comparing replies from different servers to extract
	 *            one returned by f+1
	...
	public byte[] invoke(byte[] request, TOMMessageType reqType) {
		...
		if (response == null) {
			...
			if (reqType == TOMMessageType.UNORDERED_REQUEST || reqType == TOMMessageType.UNORDERED_HASHED_REQUEST) {
				...
				throw new RuntimeException("Received n-f replies without f+1 of them matching."); //TODO check replies of replicas
			}
		} else {
			// normal operation
			// ******* EDUARDO BEGIN **************//
