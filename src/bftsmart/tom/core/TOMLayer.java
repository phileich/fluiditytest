...
public final class TOMLayer extends Thread implements RequestReceiver {
	...
	private ServerCommunicationSystem communication; // Communication system
														// between replicas
	// private OutOfContextMessageThread ot; // Thread which manages messages
	// that do not belong to the current consensus
	private DeliveryThread dt; // Thread which delivers total ordered messages
								// to the appication
	public StateManager stateManager = null; // object which deals with the
	...
	// the next two are used to generate non-deterministic data in a
	// deterministic way (by the leader)
	...
	public TOMLayer(ExecutionManager manager, ServiceReplica receiver, Recoverable recoverer, Acceptor a,
		...
		if (this.controller.getStaticConf().getRequestTimeout() == 0) {
			...
			this.requestsTimer = new RequestsTimer(this, communication, this.controller); // Create
																							// requests
																							// timers
																							// manager
																							// (a
		...
		try {
			this.md = MessageDigest.getInstance("MD5"); // TODO: shouldn't it be
														// SHA?
		} catch (Exception e) {
		...
		this.dt = new DeliveryThread(this, receiver, recoverer, this.controller); // Create
																					// delivery
																					// thread
		this.dt.start();
	...
	public byte[] createPropose(Decision dec) {
		...
		int numberOfMessages = pendingRequests.size(); // number of messages
														// retrieved
		int numberOfNonces = this.controller.getStaticConf().getNumberOfNonces(); // ammount
																					// of
																					// nonces
																					// to
																					// be
		...
		// for benchmarking
		if (dec.getConsensusId() > -1) { // if this is from the leader change,
											// it doesnt matter
		...
		try {
			...
			if (addToClientManager) {
				for (int i = 0; i < requests.length; i++) {
					// notifies the client manager that this request was
					// received and get
