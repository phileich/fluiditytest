...
public class ServersCommunicationLayer extends Thread {
	...
	public ServersCommunicationLayer(ServerViewController controller,
		...
		// Try connecting if a member of the current view. Otherwise, wait until
		// the Join has been processed!
		...
		serverSocket = new ServerSocket(
				controller.getStaticConf().getServerToServerPort(
						controller.getStaticConf().getProcessId()),
				0, InetAddress.getByName(controller.getStaticConf()
						.getHost(controller.getStaticConf().getProcessId())));
		/* serverSocket = new ServerSocket(
				controller.getStaticConf().getServerToServerPort(
						controller.getStaticConf().getProcessId())); */
	...
	private ServerConnection getConnection(int remoteId) {
		...
		if (ret == null) {
			ret = new ServerConnection(controller, null, remoteId, this.inQueue,
					this.replica);
	...
	public void run() {
		while (doWork) {
			try {
				...
				// ******* EDUARDO BEGIN **************//
				if (!this.controller.isInCurrentView() && (this.controller
						.getStaticConf().getTTPId() != remoteId)) {
					waitViewLock.lock();
	...
	// ******* EDUARDO BEGIN **************//
	private void establishConnection(Socket newSocket, int remoteId)
			throws IOException {
		if ((this.controller.getStaticConf().getTTPId() == remoteId)
			...
			if (this.connections.get(remoteId) == null) { // This must never
															// happen!!!
				// first time that this connection is being established
				// System.out.println("THIS DOES NOT HAPPEN....."+remoteId);
				this.connections.put(remoteId, new ServerConnection(controller,
						newSocket, remoteId, inQueue, replica));
			} else {
				// reconnection
	...
	public String toString() {
		...
		for (int i = 0; i < activeServers.length; i++) {
			...
			// if(connections[i] != null) {
			if (me != activeServers[i]) {
				str += ", connections[" + activeServers[i] + "]: outQueue="
						+ getConnection(activeServers[i]).outQueue;
			}
		}
