...
public class ServerConnection {
	...
	protected LinkedBlockingQueue<byte[]> outQueue;// = new
													// LinkedBlockingQueue<byte[]>(SEND_QUEUE_SIZE);
	private HashSet<Integer> noMACs = null; // this is used to keep track of
											// data to be sent without a MAC.
											// It uses the reference id for that
											// same data
	...
	public ServerConnection(ServerViewController controller, Socket socket,
			int remoteId, LinkedBlockingQueue<SystemMessage> inQueue,
		...
		this.outQueue = new LinkedBlockingQueue<byte[]>(
		...
		if (isToConnect()) {
			...
			try {
				...
				this.socket = new Socket(
						InetAddress.getByName(this.controller.getStaticConf()
								.getHost(remoteId)),
						this.controller.getStaticConf()
								.getServerToServerPort(remoteId),
						InetAddress.getByName(
								this.controller.getStaticConf().getHost(controller.getStaticConf().getProcessId())),
						0);
				/*
				 * this.socket = new Socket(
				...
				new DataOutputStream(this.socket.getOutputStream()).writeInt(
		...
		if (this.socket != null) {
			try {
				...
				socketInStream = new DataInputStream(
		...
		// ******* EDUARDO BEGIN **************//
		this.useSenderThread = this.controller.getStaticConf()
				.isUseSenderThread();
	 ...
	 */
	public final void send(byte[] data, boolean useMAC)
			throws InterruptedException {
		if (useSenderThread) {
			// only enqueue messages if there queue is not full
			if (!useMAC) {
				Logger.println("(ServerConnection.send) Not sending defaultMAC "
						+ System.identityHashCode(data));
				noMACs.add(System.identityHashCode(data));
			}

			if (!outQueue.offer(data)) {
				Logger.println("(ServerConnection.send) out queue for "
						+ remoteId + " full (message discarded).");
			}
		} else {
			sendLock.lock();
	...
	private final void sendBytes(byte[] messageData, boolean useMAC) {
		...
		do {
			...
			if (socket != null && socketOutStream != null) {
				try {
					// do an extra copy of the data to be sent, but on a single
					// out stream write
					byte[] mac = (useMAC && this.controller.getStaticConf()
							.getUseMACs() == 1) ? macSend.doFinal(messageData)
									: null;
					byte[] data = new byte[5 + messageData.length
							+ ((mac != null) ? mac.length : 0)];
					int value = messageData.length;

					System.arraycopy(new byte[] { (byte) (value >>> 24),
							(byte) (value >>> 16), (byte) (value >>> 8),
							(byte) value }, 0, data, 0, 4);
					System.arraycopy(messageData, 0, data, 4,
							messageData.length);
					if (mac != null) {
						// System.arraycopy(mac,0,data,4+messageData.length,mac.length);
						System.arraycopy(new byte[] { (byte) 1 }, 0, data,
								4 + messageData.length, 1);
						System.arraycopy(mac, 0, data, 5 + messageData.length,
								mac.length);
					} else {
						System.arraycopy(new byte[] { (byte) 0 }, 0, data,
								4 + messageData.length, 1);
					}
	...
	// ******* EDUARDO BEGIN **************//
	// return true of a process shall connect to the remote process, false
	// otherwise
	private boolean isToConnect() {
		if (this.controller.getStaticConf().getTTPId() == remoteId) {
			// Need to wait for the connection request from the TTP, do not tray
			// to connect to it
			return false;
		} else if (this.controller.getStaticConf().getTTPId() == this.controller
		...
		if (this.controller.isInCurrentView()) {
			 ...
			 * from that same code (its the only part I understand why is
			 * necessary) I keep the code commented just to be on the safe side
			 */

			/**
			 * 
			 * boolean me =
			 * this.controller.isInLastJoinSet(this.controller.getStaticConf().
			 * getProcessId()); boolean remote =
			 * this.controller.isInLastJoinSet(remoteId);
			 * 
			 * //either both endpoints are old in the system (entered the system
			 * in a previous view), //or both entered during the last
			 * reconfiguration if ((me && remote) || (!me && !remote)) { //in
			 * this case, the node with higher ID starts the connection if
			 * (this.controller.getStaticConf().getProcessId() > remoteId) { ret
			 * = true; } //this process is the older one, and the other one
			 * entered in the last reconfiguration } else if (!me && remote) {
			 * ret = true;
			 * 
			 * } //else if (me && !remote) { //this process entered in the last
			 * reconfig and the other one is old //ret=false; //not necessary,
			 * as ret already is false //}
			 * 
			 */
		}
		return ret;
	 ...
	 *
	 * @param newSocket
	 *            socket created when this server accepted the connection (only
	 *            used if processId is less than remoteId)
	 */
		...
		if (socket == null || !socket.isConnected()) {
			...
			try {
				...
				if (isToConnect()) {
					// TODO: SOCKET
					socket = new Socket(
							InetAddress.getByName(this.controller
									.getStaticConf().getHost(remoteId)),
							this.controller.getStaticConf()
									.getServerToServerPort(remoteId),
							InetAddress.getByName(this.controller
									.getStaticConf().getHost(controller
											.getStaticConf().getProcessId())),
							0);
					/*
					 * socket = new Socket(
					...
					new DataOutputStream(socket.getOutputStream()).writeInt(
							this.controller.getStaticConf().getProcessId());
				...
				System.out.println(
						"Impossible to reconnect to replica " + remoteId);
				// ex.printStackTrace();
			}
	...
	public void authenticateAndEstablishAuthKey() {
		if (authKey != null || socketOutStream == null
				|| socketInStream == null) {
			return;
		}
		...
		try {
			...
			// } else {
			// I received a connection request, so I'm second on the auth
			// protocol
			...
			PrivateKey RSAprivKey = controller.getStaticConf()
					.getRSAPrivateKey();
			BigInteger DHPrivKey = new BigInteger(RSAprivKey.getEncoded());

			// Create DH public key
			BigInteger myDHPubKey = controller.getStaticConf().getDHG()
					.modPow(DHPrivKey, controller.getStaticConf().getDHP());
			...
			// verify signature
			PublicKey remoteRSAPubkey = controller.getStaticConf()
					.getRSAPublicKey(remoteId);

			if (!TOMUtil.verifySignature(remoteRSAPubkey, remote_Bytes,
					remote_Signature)) {
			...
			// Create secret key
			BigInteger secretKey = remoteDHPubKey.modPow(DHPrivKey,
			...
			PBEKeySpec spec = new PBEKeySpec(
					secretKey.toString().toCharArray());
	...
	private class SenderThread extends Thread {
		...
		public void run() {
			...
			while (doWork) {
				...
				if (data != null) {
					// sendBytes(data,
					// noMACs.contains(System.identityHashCode(data)));
					int ref = System.identityHashCode(data);
					boolean sendMAC = !noMACs.remove(ref);
					Logger.println("(ServerConnection.run) "
							+ (sendMAC ? "Sending" : "Not sending")
							+ " MAC for data " + ref);
	...
	protected class ReceiverThread extends Thread {
		...
		public void run() {
			...
			try {
				receivedMac = new byte[Mac.getInstance(MAC_ALGORITHM)
						.getMacLength()];
			...
			while (doWork) {
				if (socket != null && socketInStream != null) {
					try {
						...
						int read = 0;
						do {
							read += socketInStream.read(data, read,
									dataLength - read);
						} while (read < dataLength);
						...
						if (controller.getStaticConf().getUseMACs() == 1
							...
							read = 0;
							do {
								read += socketInStream.read(receivedMac, read,
										macSize - read);
							} while (read < macSize);

							result = Arrays.equals(macReceive.doFinal(data),
									receivedMac);
						}

						if (result) {
							SystemMessage sm = (SystemMessage) (new ObjectInputStream(
									new ByteArrayInputStream(data))
											.readObject());
							sm.authenticated = (controller.getStaticConf()
									.getUseMACs() == 1 && hasMAC == 1);

							if (sm.getSender() == remoteId) {
								if (!inQueue.offer(sm)) {
									Logger.println(
											"(ReceiverThread.run) in queue full (message from "
													+ remoteId
													+ " discarded).");
									System.out.println(
											"(ReceiverThread.run) in queue full (message from "
							...
							// TODO: violation of authentication... we should do
							// something
							Logger.println(
									"WARNING: Violation of authentication in message received from "
											+ remoteId);
						}
	...
	// ******* EDUARDO BEGIN: special thread for receiving messages indicating
	// the entrance into the system, coming from the TTP **************//
	// Simly pass the messages to the replica, indicating its entry into the
	// system
				...
				if (socket != null && socketInStream != null) {
					try {
						...
						if (result) {
							...
							if (sm.getSender() == remoteId) {
								// System.out.println("Mensagem recebia de:
								// "+remoteId);
								/*
								 * if (!inQueue.offer(sm)) {
