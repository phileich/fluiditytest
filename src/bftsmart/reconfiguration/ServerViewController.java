...
public class ServerViewController extends ViewController {
	...
	public ServerViewController(int procId) {
		this(procId, "");
		/*
		 * super(procId); initialView = new View(0,
		 * getStaticConf().getInitialView(), getStaticConf().getF(),
		 * getInitAdddresses()); getViewStore().storeView(initialView);
	...
	public ServerViewController(int procId, String configHome) {
		...
		if (cv == null) {
			...
			reconfigureTo(new View(0, getStaticConf().getInitialView(), getStaticConf().getF(), getInitAdddresses(),
					getStaticConf().isBFT(), getStaticConf().getDelta(), getStaticConf().useFluidity()));
		} else {
	...
	private byte[] reconfigure(List<String> jSetInfo, List<Integer> jSet, List<Integer> rSet, int f, int cid) {
		...
		View newV = new View(currentView.getId() + 1, nextV, f, addresses, getStaticConf().isBFT(),
				getStaticConf().getDelta(), getStaticConf().useFluidity());
