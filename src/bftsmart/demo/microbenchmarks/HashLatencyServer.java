 ...
 */
public final class HashLatencyServer extends DefaultRecoverable {
	...
	public HashLatencyServer(int id, int interval, int replySize, int stateSize,
			int hashOperations, boolean context) {
		if (hashOperations < 0) {
			throw new IllegalArgumentException(
					"Hash opertaions count must positive");
		}
	...
	public byte[] execute(byte[] command, MessageContext msgCtx) {
		...
		if (msgCtx != null && msgCtx.getFirstInBatch() != null) {
			...
			totalLatency.store(msgCtx.getFirstInBatch().executedTime
					- msgCtx.getFirstInBatch().receptionTime);

			if (readOnly == false) {

				consensusLatency.store(msgCtx.getFirstInBatch().decisionTime
						- msgCtx.getFirstInBatch().consensusStartTime);
				long temp = msgCtx.getFirstInBatch().consensusStartTime
						- msgCtx.getFirstInBatch().receptionTime;
				preConsLatency.store(temp > 0 ? temp : 0);
				posConsLatency.store(msgCtx.getFirstInBatch().executedTime
						- msgCtx.getFirstInBatch().decisionTime);
				proposeLatency.store(msgCtx.getFirstInBatch().writeSentTime
						- msgCtx.getFirstInBatch().consensusStartTime);
				writeLatency.store(msgCtx.getFirstInBatch().acceptSentTime
						- msgCtx.getFirstInBatch().writeSentTime);
				acceptLatency.store(msgCtx.getFirstInBatch().decisionTime
						- msgCtx.getFirstInBatch().acceptSentTime);
		...
		if (hashOperations > 0) {
			...
			try {
				...
				byte[] hash = md.digest(command);

				for (int i = 1; i < hashOperations; i++) {
					hash = md.digest(command);
				}
		...
		if (iterations % interval == 0) {
			if (context)
				System.out.println("--- (Context)  iterations: " + iterations
						+ " // regency: " + msgCtx.getRegency()
						+ " // consensus: " + msgCtx.getConsensusId() + " ---");

			System.out.println("--- Measurements after " + iterations + " ops ("
					+ interval + " samples) ---");

			tp = (float) (interval * 1000 / (float) (System.currentTimeMillis()
					- throughputMeasurementStartTime));

			if (tp > maxTp)
				maxTp = tp;

			System.out.println(
					"Throughput = " + tp + " operations/sec (Maximum observed: "
							+ maxTp + " ops/sec)");

			System.out.println("Total latency = "
					+ totalLatency.getAverage(false) / 1000 + " (+/- "
			...
			System.out.println("Propose latency = "
					+ proposeLatency.getAverage(false) / 1000 + " (+/- "
					+ (long) proposeLatency.getDP(false) / 1000 + ") us ");
			proposeLatency.reset();
			System.out.println("Write latency = "
					+ writeLatency.getAverage(false) / 1000 + " (+/- "
					+ (long) writeLatency.getDP(false) / 1000 + ") us ");
			writeLatency.reset();
			System.out.println("Accept latency = "
					+ acceptLatency.getAverage(false) / 1000 + " (+/- "
	...
	public static void main(String[] args) {
		if (args.length < 5) {
			System.out.println(
					"Usage: ... ThroughputLatencyServer <processId> <measurement interval> <reply size> <state size> <hash operations> <context?>");
			System.exit(-1);
		}
		...
		int hashOperations = Integer.parseInt(args[4]);
		boolean context = Boolean.parseBoolean(args[5]);

		new HashLatencyServer(processId, interval, replySize, stateSize,
				hashOperations, context);
	}
